...
for (simulation in c("01", "02", "03")){
  ...
  for(i in 1:length(df_list)){
    ...
    df_long_list[[i]]$value <- log(df_long_list[[i]]$value)
    df_long_list[[i]]$true <- log(df_true_long$value)
    df_long_list[[i]]$diff <- df_long_list[[i]]$value - df_long_list[[i]]$true
    df_long_list[[i]] <- df_long_list[[i]][df_long_list[[i]]$diff > -5,]
    df_long_list[[i]] <- df_long_list[[i]][df_long_list[[i]]$diff < 5,]
  }
...
names <- c("1 character", "2 character", "3 character")
ggplot(df_out[[1]],
...
# write.csv(raw_dat, "tables/log-parameter-comparison.csv", row.names = FALSE)
...
# write.csv(final_df, "tables/log-par-summary-by-type.csv", row.names = FALSE)
...
write.csv(stats_df, "tables/out-rm-log-stats-summary-by-type.csv", row.names = FALSE)

prop_var_df <- rbind(prop_df1, prop_df2, prop_df3)
prop_var_df[,-1] * 100
write.csv(prop_var_df, "tables/log-propor-variance-by-type.csv", row.names = FALSE)
# with 100 trans / MY
dat <- do.call(rbind, df_out)
# Apply the aggregate function
...
write.csv(final_df, "tables/log-par-summary-by-type-par.csv", row.names = FALSE)
...
write.csv(final_df, "tables/log-par-summary-by-type-model.csv", row.names = FALSE)
